...
export function TodoItem({ id, task, reminderTime, category, status, completed }: TodoProps) {
  ...
  return (
    <div className={`group flex flex-col md:flex-row md:items-center justify-between p-6 rounded-[2.5rem] bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 hover:shadow-xl hover:shadow-indigo-500/5 transition-all duration-300 ${isCompleted ? 'opacity-80' : ''}`}>
      <div className="flex items-center gap-5 mb-5 md:mb-0">
        <button
          onClick={toggleComplete}
          disabled={loading}
          className={`h-9 w-9 rounded-2xl border-2 transition-all flex items-center justify-center shrink-0 disabled:opacity-50 ${isCompleted
      ...
      <div className="flex flex-col sm:flex-row items-center gap-4">
        ...
        <div className={`flex items-center gap-3 px-6 py-3 rounded-2xl border transition-all duration-500 ${
            timeLeft === "Time's up!" ? "bg-red-50 border-red-100 dark:bg-red-500/10 dark:border-red-500/20" : 
          ...
          <span className={`text-sm font-mono font-black ${
            timeLeft === "Time's up!" ? "text-red-500 animate-pulse" : 
